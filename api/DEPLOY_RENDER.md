...
1) Create a new Web Service on Render
   - Connect your GitHub repo and use the Dockerfile-based service (select api/Dockerfile).
   - Alternatively, for a non-Docker deployment, set "Root Directory" to `api/`, "Build Command" to `pip install -r requirements.txt`, and "Start Command" to `uvicorn main:app --host 0.0.0.0 --port $PORT`.
